...
while getopts "h?d:" opt; do
	case "$opt" in
	h|\?)
	  echo "Usage: ./checketh1.sh [option]"
	  echo "Options:"
	  echo " -d - console port"
	  echo "Example: checketh1.sh -d /dev/ttyUSB0"
	  exit 0
	;;
	d) device=$OPTARG
	;;
...
while [ "$COUNT" -ne "$IF_COUNT" ]; do
	...
	if [ -n "$ETH" ]; then
		NEEDED_IF="$IF"
		echo "ETH1 connected to $IF interface"
	else
		ifconfig $IF down
	fi
	sleep 2
...
if [ -n "$NEEDED_IF" ]; then
	echo "ETH1: trying to ping 8.8.8.8"
	RESULT=""
	...
	echo "ETH1: trying to ping 8.8.4.4"
	RESULT=""
